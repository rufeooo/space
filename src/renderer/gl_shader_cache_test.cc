...
constexpr const char* kInvalidVertexShaderName = "invalid_vertex_shader";
...
constexpr const char* kValidFragmentShaderName = "valid_fragment_shader";

constexpr const char* kValidProgramName = "valid_fragment_shader";
...
TEST_F(GLShaderCacheTest, HappyPathShaderCompilation)
{
  ...
  uint32_t shader_ref;
  ASSERT_FALSE(
      shader_cache.GetShaderReference(kValidVertexShaderName, &shader_ref));
  ASSERT_TRUE(shader_cache.CompileShader(
      kValidVertexShaderName, ShaderType::VERTEX, kValidVertexShaderSrc));
  ASSERT_TRUE(
      shader_cache.GetShaderReference(kValidVertexShaderName, &shader_ref));
}
...
TEST_F(GLShaderCacheTest, HappyPathShaderCompilationAndLink)
{
  ...
  ASSERT_TRUE(shader_cache.CompileShader(
      kValidVertexShaderName, ShaderType::VERTEX, kValidVertexShaderSrc));
  ASSERT_TRUE(shader_cache.CompileShader(
      kValidFragmentShaderName, ShaderType::FRAGMENT, kValidFragmentShaderSrc));
  uint32_t shader_ref;
  ASSERT_TRUE(
      shader_cache.GetShaderReference(kValidVertexShaderName, &shader_ref));
  ASSERT_TRUE(
      shader_cache.GetShaderReference(kValidFragmentShaderName, &shader_ref));
  ...
  ASSERT_TRUE(shader_cache.LinkProgram(
      kValidProgramName, {kValidVertexShaderName, kValidFragmentShaderName}));
  uint32_t program_ref;
  ASSERT_TRUE(
      shader_cache.GetProgramReference(kValidProgramName, &program_ref));
}

TEST_F(GLShaderCacheTest, ErrorOnRecompilingSameShaderSourceDifferentName)
{
  GLShaderCache shader_cache;
  ASSERT_TRUE(shader_cache.CompileShader(
      kValidVertexShaderName, ShaderType::VERTEX, kValidVertexShaderSrc));
  ASSERT_FALSE(shader_cache.CompileShader("different_name", ShaderType::VERTEX,
...
TEST_F(GLShaderCacheTest, ErrorCompilingInvalidShaderSource)
{
  ...
  ASSERT_FALSE(shader_cache.CompileShader(
      kInvalidVertexShaderName, ShaderType::VERTEX, kInvalidVertexShaderSrc));
}
...
TEST_F(GLShaderCacheTest, ShaderProgramInfo)
{
  ...
  ASSERT_TRUE(shader_cache.LinkProgram(
      kValidProgramName, {kValidVertexShaderName, kValidFragmentShaderName}));
  ASSERT_EQ(shader_cache.GetProgramInfo(kValidProgramName),
