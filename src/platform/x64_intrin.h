...
inline uint64_t TARGET("bmi") BLSR(uint64_t f) { return _blsr_u64(f); }

// Right 0s: Trailing zero count
inline uint64_t TARGET("bmi") TZCNT(uint64_t f) { return _tzcnt_u64(f); }

// Left 0s: Leading zero count
inline uint64_t TARGET("lzcnt") LZCNT(uint64_t f) { return _lzcnt_u64(f); }
...
// Logically: ~a & f
inline uint64_t TARGET("bmi") ANDN(uint64_t a, uint64_t f) { return _andn_u64(a, f); }
