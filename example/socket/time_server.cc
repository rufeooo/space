...
int main() {
  ...
  printf("  socket_listen = socket(bind_address->ai_family, "
  ...
  printf("  bind(socket_listen, bind_address->ai_addr, "
         "bind_address->ai_addrlen)\n\n");
  ...
  printf("  socket_client = accept(socket_listen, (struct sockaddr*) "
  ...
  printf("  getnameinfo((struct sockaddr*)&client_address, "
  ...
  char request[1024];
  printf("  bytes_received = recv(socket_client, request, 1024, "
         "0)\n\n");
  ...
  const char *response =
      ...
      "Local time is: ";
  printf("  bytes_sent = send(socket_client, response, "
         "strlen(response), 0)\n\n");
  ...
  printf("  bytes_sent = send(socket_client, time_msg, "
         "strlen(time_msg), 0)\n\n");
