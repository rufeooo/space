...
class EntityReplication
{
 public:
  ...
  // create the entity.
  void AddEntityAuthority(ecs::Entity, std::vector<uint8_t>&& command_data);
 ...
 private:
  ...
  std::unordered_map<ecs::Entity, std::vector<uint8_t>> authority_entities_;
  ...
  // client_.
  std::unordered_map<int, std::unordered_set<ecs::Entity>> client_entities_;
